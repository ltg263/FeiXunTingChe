...
import com.jxkj.fxtc.conpoment.view.DataModel;
import com.jxkj.fxtc.conpoment.view.ScrollBlockView;
...
public class MainChartConfig {
    ...
    public void setData(List<DataModel> dataModels) {
        setData(dataModels,0);
    }
    public void setData(List<DataModel> dataModels,int size){
        parseEntry(dataModels,size);
        setMainLineData();
        refresh();
    }
     ...
     * 解析数据
     */
    private void parseEntry(List<DataModel> models, int size) {
        // 清除旧的数据
        entryList.clear();
        colorList.clear();
        ...
        if (models != null && models.size() > 0) {
            ...
            for (int i = 0; i < models.size(); i++) {
                ...
                // 根据y值，设置点的颜色
                if (model.y <= 50) {
                    colorList.add(mContext.getResources().getColor(R.color.color_ffffff));
                } else if (model.y <= 100) {
                    colorList.add(mContext.getResources().getColor(R.color.color_ffffff));
                } else {
                    colorList.add(mContext.getResources().getColor(R.color.color_ffffff));
            ...
            updateY(max, min,size);
            updateX(models);
        } else {
            // 设置一个超出范围的值，保证列表不空，可以解决空数据下，不触发绘制的问题
            entryList.add(new Entry(200000000, 150));
        }
    }

    private void updateX(List<DataModel> models) {
        XAxis xAxis = mMainChart.getXAxis();
        // 设置最大值，最小值
        xAxis.setAxisMaximum(models.size());
        ...
        xAxis.setValueFormatter(new ValueFormatter() {
            ...
            public String getFormattedValue(float value) {
                ...
                int v = (int) value;
                if (v < models.size() && v >= 0) {
                    String st = models.get(v).time;
                    String tim1 = "";
                    tim1 = st;
                    return tim1;
                } else {
     ...
     * 根据图表中的最值进行自适应Y轴的高度
     */
    private void updateY(float max, float min,float size) {
        if (max <= 10) {
            // 最大值不低于15
            max = 15;
        } else {
        ...
        mMainChart.getAxisLeft().setAxisMaximum(size);
        mMainChart.getAxisLeft().setAxisMinimum(0);
    }
    ...
    private void setMainXConfig() {
        ...
        xAxis.setTextColor(mContext.getResources().getColor(R.color.color_ffffff));
        // 不要轴线
        xAxis.setDrawAxisLine(false);
        // 不要网格线
        xAxis.setDrawGridLines(true);
        xAxis.setGranularityEnabled(false);
        xAxis.setGridColor(mContext.getResources().getColor(R.color.color_D5DAE6));
    ...
    private void setMainYConfig() {
        ...
        yAxis.setTextColor(mContext.getResources().getColor(R.color.transparent));
        // 不要轴线
        ...
        yAxis.setGridColor(mContext.getResources().getColor(R.color.color_D5DAE6));
