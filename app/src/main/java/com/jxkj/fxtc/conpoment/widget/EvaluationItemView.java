...
public class EvaluationItemView extends View {
    ...
    public EvaluationItemView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void init(Context context, AttributeSet attrs) {
        ...
        mLineColor = typedArray.getColor(R.styleable.RulerView_rvLineColor, mLineColor);

        mTextSize = typedArray.getDimension(R.styleable.RulerView_rvTextSize, dp2px(context, mTextSize));
        mTextColor = typedArray.getColor(R.styleable.RulerView_rvTextColor, mTextColor);
...
//            }
            ...
            //绘制刻度尺的竖线
            canvas.drawLine(left, height*3, left, mLineMax + mTextMarginTop + mTextHeight, mLinePaint);
            mLinePaint.setStrokeWidth(2);
            if(i<mTotalLine-1){
                //绘制刻度尺间隔的横线
                //canvas.drawLine(left, mLineMax + mTextMarginTop + mTextHeight, left+mLineSpaceWidth, mLineMax + mTextMarginTop + mTextHeight, mLinePaint);
            }
            if (i% 10 == 0) {
                ...
                //mLineMax + mTextMarginTop + mTextHeight
                //绘制刻度尺下面的文字
                canvas.drawText(value+unit, left - mTextPaint.measureText(value) / 2,
                        height*3-5, mTextPaint);
            }
        }
        if(mAlphacolor!=0){
            mLinePaint.setColor(getResources().getColor(mAlphacolor));
        }else{
            mLinePaint.setColor(getResources().getColor(R.color.color_ff9c3c));
        }
        mLinePaint.setStrokeWidth(3);
        //绘制屏幕中间的那条红色竖线
        canvas.drawLine(mWidth/2,mTextMarginTop + mTextHeight - 10,mWidth/2,mLineMax + mTextMarginTop + mTextHeight,mLinePaint);
    }
