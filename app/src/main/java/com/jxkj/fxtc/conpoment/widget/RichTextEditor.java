...
//        GlideUtils.loadSideMaxImage(context, imageView, imagePath);
     ...
     * 生成控件中的数据
     */
    public static String getEditData(List<EditData> editList) {
     ...
     * 生成控件中的数据
     */
    public static boolean isBlankEd(List<EditData> editList) {
        if (editList.size() > 0) {
            for (RichTextEditor.EditData itemData : editList) {
...
//                }
     ...
     * 生成控件中的数据
     */
    public static boolean isBlankEdLeng(List<EditData> editList) {
        if (editList.size() > 0) {
            int length = 0;
            for (RichTextEditor.EditData itemData : editList) {
                if (StringUtil.isNotBlank(replaceBlank(itemData.inputStr))) {
     ...
     * 生成控件中的数据
     */
    public static int getBlankEdLeng(List<EditData> editList) {
        int length = 0;
        if (editList.size() > 0) {
            for (RichTextEditor.EditData itemData : editList) {
                 length += replaceBlank(itemData.inputStr).length();
